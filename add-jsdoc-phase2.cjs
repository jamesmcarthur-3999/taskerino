...
console.log('Phase 2: Adding comprehensive JSDoc to types.ts...\n');
...
const sectionDocs = [
  {
    pattern: /^export interface AchievementsSection extends BaseSummarySection \{$/m,
    doc: `/**
...
export interface AchievementsSection extends BaseSummarySection {`
  ...
  {
    pattern: /^export interface BreakthroughMomentsSection extends BaseSummarySection \{$/m,
    doc: `/**
...
export interface BreakthroughMomentsSection extends BaseSummarySection {`
  ...
  {
    pattern: /^export interface BlockersSection extends BaseSummarySection \{$/m,
    doc: `/**
...
// Apply replacements
for (const {pattern, doc} of sectionDocs) {
...
console.log('\nPhase 2 JSDoc additions applied successfully!');
console.log(`Restore backup with: mv ${backupPath} ${filePath}`);
