...
const SOURCE_COLORS = {
  call: 'bg-blue-100/70 text-blue-700',
  email: 'bg-cyan-100/70 text-cyan-700',
  thought: 'bg-green-100/70 text-green-700',
...
export const NoteCard = React.memo<NoteCardProps>(({
  ...
  if (compact) {
    return (
      <motion.div
        ...
        className="px-3 py-2 rounded-lg bg-white/70 backdrop-blur-xl border-2 border-white/60 shadow-lg shadow-cyan-100/30 cursor-pointer hover:border-cyan-300/60 hover:shadow-cyan-200/40 transition-all"
  ...
  return (
    <motion.div
      ...
      className="rounded-xl border-2 border-white/60 bg-white/70 backdrop-blur-xl overflow-hidden shadow-lg shadow-cyan-100/30 hover:shadow-xl hover:shadow-cyan-200/40 transition-all"
    >
      {/* Header */}
            ...
            {(companies.length > 0 || contacts.length > 0) && (
              <div className="flex flex-wrap items-center gap-2 mt-2">
                {companies.map((company) => (
                  <span
                    key={company.id}
                    className="flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-blue-100/70 text-blue-700 hover:bg-blue-200/70 transition-colors cursor-pointer"
                  >
                ...
                {contacts.map((contact) => (
                  <span
                    key={contact.id}
                    className="flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-green-100/70 text-green-700 hover:bg-green-200/70 transition-colors cursor-pointer"
                  >
      ...
      <div className="p-4">
        <div className="prose prose-sm max-w-none">
          <p className="text-gray-700 whitespace-pre-wrap">
            {highlightedParts.map((part, idx) => {
              if (part.type === 'date') {
                return (
                  <span key={idx} className="px-1 py-0.5 bg-blue-100/70 text-blue-700 rounded font-medium">
                    {part.text}
                  </span>
                );
              } else if (part.type === 'action') {
                return (
                  <span key={idx} className="px-1 py-0.5 bg-orange-100/70 text-orange-700 rounded font-medium">
                    {part.text}
                  </span>
                );
              }
        ...
        {canExpand && (
          <button
            ...
            className="flex items-center gap-1 mt-3 text-sm text-cyan-600 hover:text-cyan-700 transition-colors"
          >
            {isExpanded ? (
              <>
          ...
          <AnimatePresence>
            {showKeyPoints && (
              <motion.div
                ...
                className="p-3 rounded-lg bg-gradient-to-br from-cyan-50/70 to-blue-50/70 border border-cyan-100/60"
              >
            ...
            {showRelatedTasks && (
              ...
              >
                {relatedTasks.map((task) => (
                  <div
                    key={task.id}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/50 border border-white/60 hover:border-cyan-300/60 transition-all text-xs"
                  >
                    {task.done ? (
        ...
        <div className="px-4 pb-4">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {note.tags.map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 text-xs rounded-full bg-cyan-100/70 text-cyan-700 hover:bg-cyan-200/70 transition-colors cursor-pointer"
              >
                {tag}
              </span>
            ))}
            ...
            {showUpdates && (
              ...
              >
                {note.updates.slice(0, 3).map((update) => (
                  <div
                    key={update.id}
                    className="flex gap-2 px-3 py-2 rounded-lg bg-white/50 border border-white/60"
                  >
