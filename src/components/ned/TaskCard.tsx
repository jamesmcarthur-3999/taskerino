...
import { getGlassClasses, getRadiusClass, TRANSITIONS, getPriorityColors } from '../../design-system/theme';
  ...
  if (compact) {
    return (
      <motion.div
        ...
        className={`flex items-center gap-2 px-3 py-2 ${getRadiusClass('field')} ${getGlassClasses('medium')} hover:shadow-cyan-200/40 ${TRANSITIONS.standard}`}
      >
        <button
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start gap-2">
            ...
            {task.createdBy === 'ai' && task.aiContext && (
              <div
                className={`flex items-center gap-1 px-2 py-0.5 ${getRadiusClass('pill')} bg-purple-100 text-purple-800 text-xs`}
          ...
          <div className="flex flex-wrap items-center gap-3 mt-3">
            ...
            <button
              ...
              className={`flex items-center gap-1 text-xs font-medium ${PRIORITY_COLORS[task.priority]} hover:scale-105 ${TRANSITIONS.transform}`}
                ...
                <AnimatePresence>
                  {showReschedule && onReschedule && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -5 }}
                      className={`absolute top-full mt-1 left-0 z-10 flex gap-1 p-1 ${getGlassClasses('extra-strong')} ${getRadiusClass('field')} shadow-xl`}
                    >
                      <button
                        onClick={() => handleQuickReschedule(0)}
                        className={`px-2 py-1 text-xs rounded hover:bg-cyan-100 ${TRANSITIONS.colors}`}
                      >
                        Today
                      </button>
                      <button
            ...
            {subtaskText && (
              <button
                ...
                className={`flex items-center gap-1.5 text-xs text-gray-600 hover:text-cyan-600 ${TRANSITIONS.colors}`}
              >
            ...
            {task.tags && task.tags.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {task.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className={`px-2 py-0.5 text-xs ${getRadiusClass('pill')} bg-cyan-100 text-cyan-800`}
                  >
                    {tag}
                  </span>
                ))}
              ...
              >
                {task.subtasks.map((subtask) => (
                  <button
                    ...
                    className={`flex items-center gap-2 text-xs text-gray-600 hover:text-gray-900 w-full text-left ${TRANSITIONS.colors} group`}
                  >
                    {subtask.done ? (
                      <CheckCircle2 className={`w-3 h-3 text-green-600 group-hover:scale-110 ${TRANSITIONS.transform}`} />
                    ) : (
                      <Circle className={`w-3 h-3 group-hover:scale-110 ${TRANSITIONS.transform}`} />
                    )}
              ...
              <AnimatePresence>
                {showSourceNote && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, height: 'auto' }}
                    exit={{ opacity: 0, height: 0 }}
                    className={`mt-1.5 p-2 ${getRadiusClass('field')} bg-white/50 border border-white/60`}
                  >
          ...
          {relatedNotesCount > 0 && (
            <div className="mt-2">
              <button className={`flex items-center gap-1 text-xs text-gray-600 hover:text-cyan-600 ${TRANSITIONS.colors}`}>
                <FileText className="w-3 h-3" />
